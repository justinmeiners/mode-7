...
int Mat4_UnProject(Vec3_t screenCoord, Mat4_t* matrix, Vec3_t* outVec)
{
    Mat4_t m;
    float inv[4], outv[4];
    ...
    Mat4_Inverse(matrix, &m);
    
    outv[0] = m.m[0] * inv[0] + m.m[4] * inv[1] + m.m[8] * inv[2] + m.m[12] * inv[3];
    outv[1] = m.m[1] * inv[0] + m.m[5] * inv[1] + m.m[9] * inv[2] + m.m[13] * inv[3];
    outv[2] = m.m[2] * inv[0] + m.m[6] * inv[1] + m.m[10] * inv[2] + m.m[14] * inv[3];
    outv[3] = m.m[3] * inv[0] + m.m[7] * inv[1] + m.m[11] * inv[2] + m.m[15] * inv[3];
    ...
    *outVec = Vec3_Create(outv[0] / outv[3], outv[1] / outv[3], outv[2] / outv[3]);
